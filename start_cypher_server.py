#!/usr/bin/env python3
"""
MCP Neo4j Cypher Server Starter

Simple script to start the MCP Neo4j Cypher server with HTTP transport.
...
def start_cypher_server_http(port=None):
    ...
    if port is None:
        port = int(os.getenv("CYPHER_SERVER_PORT", 8003))
    ...
    try:
        ...
        print("❌ Error: MCP Neo4j Cypher executable not found!")
        print("💡 Please install it first: pip install mcp-neo4j-cypher")
